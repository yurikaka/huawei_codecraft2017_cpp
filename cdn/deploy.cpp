...
//    for (itm = all_node.begin(); itm != all_node.end(); ++itm)
//        cout << *itm <<"," << node_actual_flow[*itm] << endl;
//    cout << endl;

    cout << current_answer.size() << endl;
    cout << "also need " << total_lost_flow << " of " << MCF.needSum << endl << endl;

//    sort(current_answer.begin(),current_answer.end());
...
//    }
//    i = 0;
//    sort(lost_direct.begin(),lost_direct.end(),cmp_lost_flow);
//    cout << lost_direct.size() << endl;
//    for (itm = lost_direct.begin(); itm != lost_direct.begin()+30; ++itm){
//        cout << *itm << ",";
//    }
//    cout << endl;
//        current_answer.push_back(*itm);
...
//            cout << itc->first << "," << i++ << endl;
    ...
    for (itm = current_answer.begin(); itm != current_answer.end(); ++itm){
        answer1 = current_answer;
        answer1.erase(answer1.begin()+(itm - current_answer.begin()));
        MCF.getTotalCost(answer1,0);
    }
    cout << all_cost << endl;

    double min_use = 0.00;
    do {
        MCF.getTotalCost(current_answer, min_use);
        min_use += 0.01;
